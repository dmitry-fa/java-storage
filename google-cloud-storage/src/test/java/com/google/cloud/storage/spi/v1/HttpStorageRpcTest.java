...
public class HttpStorageRpcTest {
  ...
  // list of MockData objects for a test case
  private static final List<MockData> TEST_MOCK_DATA = new ArrayList<>();
        ...
        public HttpTransport create() {
          return new HttpTransport() {
            ...
            protected LowLevelHttpRequest buildRequest(String method, String url)
              ...
              MockData data = TEST_MOCK_DATA.remove(0);
  ...
  public void testAbort() {
    mockResponse("DELETE", "http://example.com", new TestResponse(401, null));
    mockResponse("DELETE", "http://example.com", new TestResponse(401, null));
    mockResponse("DELETE", "http://example.com", new TestResponse(499, null));
    rpc.abort("http://example.com");
  }

  @Test
  public void testAbortNegative() {
    try {
      rpc.abort("gc://example.com");
      fail();
    } catch (IllegalArgumentException e) {
      assertEquals("java.net.MalformedURLException: unknown protocol: gc", e.getMessage());
    }

    mockResponse("DELETE", "http://example.com", new TestResponse(200, null));
    try {
      rpc.abort("http://example.com");
      fail();
    } catch (StorageException e) {
      assertEquals("reason phrase", e.getMessage());
    }
  }
}
